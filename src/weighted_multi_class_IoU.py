...
def Weighted_IoU(feature_map, ground_truth):
    '''
    ...
    for img_ind in range(len(feature_map)):
        ...
        h, w  = gt_segm.shape[0], gt_segm.shape[1]
        ...
        for i, c in enumerate(cl):
            ...
            # weight = t_i / (curr_gt_mask.shape[0] * curr_gt_mask.shape[1])

            IU[i] = (t_i * n_ii) / (t_i + n_ij - n_ii)

        mean_IU_s.append(np.sum(IU)/(h*w))
...
# print(Weighted_IoU(a, a.argmax(axis=1)))
